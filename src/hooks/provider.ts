...
const providers: Partial<Record<Chains, Promise<ethers.providers.JsonRpcProvider>>> = {};
...
export const setProvider = function (chain: Chains, provider: Promise<ethers.providers.JsonRpcProvider>) {
    providers[chain] = provider;
};

const getProvider = function (chain: Chains): Promise<ethers.providers.JsonRpcProvider> {
